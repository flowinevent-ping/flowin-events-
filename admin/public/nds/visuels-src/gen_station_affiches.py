...
def adjust(p):
    sl=p.slides[0]
    # 1) retirer le gros bandeau blanc unique
    for s in list(sl.shapes):
        if s.name=="Rounded Rectangle 11":
            s._element.getparent().remove(s._element)
    # 2) pastille blanche INDIVIDUELLE derriere chaque logo (lisibilite + bord a bord)
    logos=[s for s in sl.shapes if s.name in LOGO_NAMES]
    for pic in logos:
        padx=int(pic.width*0.12); pady=int(pic.height*0.18)
        add_card_behind(sl, pic, padx, pady)
    # 3) descendre NOS PARTENAIRES (mord sur JOUE)
    for s in sl.shapes:
