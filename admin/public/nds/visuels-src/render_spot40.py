...
def scene(t):
    ...
    if t < 3.0:                                    # INTRO — "CE SOIR / GRAND JEU" + logo NDS (façon réf 13s)
        ...
        pop(img, tl("à la clôture du festival", 50, WHITE), W / 2, H * 0.172, ramp(lt, 0.2, 0.60))
        if lt > 0.9:
            pop(img, tl("Flowin", 128, MAGENTA), W / 2, H * 0.812, ramp(lt, 0.9, 1.40))
            pop(img, tl("partenaire jeux des Nuits du Sud", 40, WHITE), W / 2, H * 0.880, ramp(lt, 1.1, 1.60))
    return img
