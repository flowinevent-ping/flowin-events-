...
(function(){
  ...
  var SE_ID = window.SE_ID || 'se-nds-2026';

  // Repli local (hors-ligne) — la source de vérité est en base : super_events.sondage_questions,
  // lue via la fonction get_sondage_questions(). Modifiable sans toucher au code + exploitable en stats.
  var QUESTIONS = [
    {id:'bv_decouverte',titre:'Découverte',type:'multi',label:"Comment as-tu connu le festival ?",options:[
    ...
    {id:'bv_motivation',titre:'Tes envies',type:'multi',max:3,label:"Qu'est-ce qui te donne envie de venir à un festival plutôt qu'un autre ?",options:[
    ...
    {id:'bv_venue',titre:'Ta venue',type:'single',label:"Comment es-tu venu·e ce soir ?",options:[
      {val:'pied',label:'À pied / vélo'},{val:'tc',label:'Bus / navette'},{val:'covoit',label:'Covoiturage'},{val:'voiture',label:'Voiture (seul·e)'}]},
    {id:'bv_acces',titre:"L'accès",type:'single',label:"L'accès au festival, ça a été ?",options:[
    ...
    {id:'bv_infos',titre:"L'info avant venue",type:'single',label:"Avant de venir, tu as trouvé les infos accès / parking / navettes ?",options:[
    ...
    {id:'bv_solutions',titre:'Laisser la voiture',type:'multi',label:"Qu'est-ce qui t'aiderait à laisser la voiture ?",options:[
    ...
    {id:'bv_mobilite_si',titre:'Venir autrement',type:'multi',label:"Je viendrais au festival autrement qu'en voiture si…",options:[
    ...
    {id:'bv_engagement',titre:'Ton engagement',type:'single',label:"Prêt·e à changer ta façon de venir, pour la planète ?",options:[
      {val:'oui',label:'Oui, facile.'},{val:'oui_si',label:'Oui, si y a des solutions'},{val:'peut',label:'Peut-être'},{val:'non',label:'Non'}]}
  ];

  // Charge la version en base (prioritaire). Si le réseau est coupé, on garde le repli local ci-dessus.
  (function loadQuestionsFromBase(){
    try{
      fetch(SUPA_URL+'/rest/v1/rpc/get_sondage_questions', {
        method:'POST',
        headers:{ 'apikey':SUPA_ANON, 'Authorization':'Bearer '+SUPA_ANON, 'Content-Type':'application/json' },
        body: JSON.stringify({ p_se: SE_ID })
      }).then(function(r){ return r.ok ? r.json() : null; })
        .then(function(d){ if (Array.isArray(d) && d.length) QUESTIONS = d; })
        .catch(function(){});
    }catch(e){}
  })();
  ...
  function renderQuestion(){
    ...
    var _lab = $('qLabel'); _lab.innerHTML = '';
    if (q.titre){
      var _t = document.createElement('div');
      _t.style.cssText = 'font-size:12.5px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:var(--amber);margin-bottom:9px';
      _t.textContent = q.titre;
      _lab.appendChild(_t);
    }
    var _q = document.createElement('div'); _q.textContent = q.label; _lab.appendChild(_q);
    if (q.type==='multi'){
      var _h = document.createElement('span');
      _h.style.cssText = 'display:block;font-size:13px;color:rgba(255,255,255,.55);font-weight:700;margin-top:8px';
