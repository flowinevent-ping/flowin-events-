...
export default function QuizSoloClient({ event, lots, partenaires, evId }: Props) {
  ...
  const couleur = event?.couleur ?? '#1D9E75'
  const isDark  = false
  ...
  return (
    <div id='flowin-vp' style={vpStyle}>
      ...
      {screen === 'landing' && (
        <div className='fl-screen active' style={{ background: '#f0faf6' }}>
          <div style={{ flex:1, display:'flex', flexDirection:'column', padding:'40px 24px 24px' }}>
            <div style={{ textAlign:'center', marginBottom:32 }}>
              ...
              <div style={{ fontSize:14, color:isDark?'rgba(255,255,255,.6)':'#888', lineHeight:1.5 }}>{event.description || 'Quiz en solo — battez le chrono !'}</div>
            </div>
            {lots.length > 0 && (
            ...
            <div style={{ marginTop:'auto' }}>
              <button className='fl-btn fl-btn-primary' style={{ background:couleur, fontSize:17 }} onClick={() => setScreen('form')}>Démarrer le quiz →</button>
              {partenaires.length > 0 && (
                <button className='fl-btn fl-btn-ghost' style={{ marginTop:10, color:'#1D9E75', borderColor:'#1D9E75' }} onClick={() => setScreen('partenaires')}>🤝 Voir nos partenaires</button>
              )}
            </div>
          </div>
        </div>
