...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  return {
    title: ev ? `${ev.nom} — QuizSolo` : 'QuizSolo Flowin',
    description: ev?.description ?? 'Quiz en solo — battez le chrono !',
  }
}

export default async function QuizSoloPage({ searchParams }: Props) {
  ...
  return (
    <QuizSoloClient event={event} lots={lots} partenaires={partenaires} evId={evId} />
  )
}
