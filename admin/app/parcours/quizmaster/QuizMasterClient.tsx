...
export default function QuizMasterClient({ event, lots, partenaires, evId }: Props) {
  ...
  const couleur = event?.couleur ?? '#6366F1'
  const isDark  = true
  ...
  return (
    <div id='flowin-vp' style={vpStyle}>
      ...
      {screen === 'landing' && (
        <div className='fl-screen active' style={{ background: '#0a0a1a' }}>
          <div style={{ flex:1, display:'flex', flexDirection:'column', padding:'40px 24px 24px' }}>
            <div style={{ textAlign:'center', marginBottom:32 }}>
              ...
              <div style={{ fontSize:14, color:isDark?'rgba(255,255,255,.6)':'#888', lineHeight:1.5 }}>{event.description || 'Quiz en direct — répondez vite !'}</div>
            </div>
            {lots.length > 0 && (
            ...
            <div style={{ marginTop:'auto' }}>
              <button className='fl-btn fl-btn-primary' style={{ background:couleur, fontSize:17 }} onClick={() => setScreen('form')}>Rejoindre le quiz live →</button>
