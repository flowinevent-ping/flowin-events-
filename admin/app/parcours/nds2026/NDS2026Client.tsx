...
export default function NDS2026Client({ ev, lots, partenaires, banques, evId }: Props) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const res = await claimJoueur(local, evId, 'ND', bonusAnswers)
      ...
      {screen === 'landing' && (
        <div className="screen" style={{ paddingTop: 32 }}>
          <div style={{ textAlign: 'center', marginBottom: 20 }}>
            <img src="/nds/logo_nds_blanc_hd.png" alt="Nuits du Sud" style={{ width:'72%',maxWidth:240,height:'auto',display:'block',margin:'0 auto 14px',filter:'drop-shadow(0 3px 12px rgba(0,0,0,.3))' }} />
            <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 8 }}>{(cfg.subtitle as string) || nom}</div>
