...
import { writeJoueur, parcoursCSS, SOURCES, SECTEURS, AGE_OPTIONS } from '@/lib/parcours'
...
export default function SpinClient({ ev, lots, partenaires, evId }: Props) {
  ...
  // Options configurables par event (master neutre par défaut)
  const isBtob       = (ev?.client_type === 'btob') || (cfg.btob === true)
  const replayable   = cfg.replayable === true
  const winTitle     = (cfg.winTitle as string) || ''
  const winCtaLabel  = (cfg.winCtaLabel as string) || ''
  const winCtaPhone  = (cfg.winCtaPhone as string) || ''
  const backUrl      = (cfg.backUrl as string) || ''
  const hidePlaceholders = cfg.hidePlaceholders === true
  ...
  useEffect(() => { if (replayable) return; try { const s = localStorage.getItem(lsKey); if (s) { setExistingTicket(s); setScreen('already') } } catch {} }, [lsKey, replayable])
  ...
  async function handleSubmit() {
    ...
    const res = await writeJoueur({ email:form.email,prenom:form.prenom,nom:form.nom,tel:form.tel,code_postal:form.cp,genre:form.genre||undefined,age_tranche:isBtob?undefined:(form.age||undefined),decouverte:form.source.replace(/^[^ ]+ /,'')||undefined,enseigne:isBtob?(form.enseigne||undefined):undefined,secteur:isBtob?(form.secteur||undefined):undefined,client_type:isBtob?'btob':undefined,events:[evId],ticket_code:tc,source:'spin',prefix:'SP' })
    setSubmitting(false)
    if (res.duplicate && !replayable) { setExistingTicket(res.ticket); try{localStorage.setItem(lsKey,res.ticket)}catch{}; setScreen('already'); return }
    setTicket(res.ticket); try{if(!replayable)localStorage.setItem(lsKey,res.ticket)}catch{}; setScreen('ticket')
  }
    ...
    <div style={{ maxWidth:430,margin:'0 auto',minHeight:'100dvh',background:'#0F172A',color:'#fff',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif' }}>
        ...
        <div className="screen">
          ...
          {isBtob && (
            <div style={{ marginBottom:12 }}>
              ...
              <input className={`input${errors.enseigne?' err':''}`} placeholder={hidePlaceholders?'':'Nom de votre commerce ou société'} value={form.enseigne} onChange={e=>setForm(f=>({...f,enseigne:e.target.value}))} />{errors.enseigne&&<div className="err">{errors.enseigne}</div>}
            </div>
          )}
          <div className="grid2" style={{ marginBottom:12 }}>
            {[['prenom','Prénom *','Camille'],['nom','Nom *','Dupont']].map(([k,l,p])=>(
              <div key={k}><label className="label">{l}</label><input className={`input${errors[k]?' err':''}`} placeholder={hidePlaceholders?'':p} value={form[k as keyof typeof form]} onChange={e=>setForm(f=>({...f,[k]:e.target.value}))} />{errors[k]&&<div className="err">{errors[k]}</div>}</div>
            ))}
          </div>
          <div style={{ marginBottom:12 }}><label className="label">Email *</label><input className={`input${errors.email?' err':''}`} type="email" placeholder={hidePlaceholders?'':'nom@exemple.fr'} autoCapitalize="none" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} />{errors.email&&<div className="err">{errors.email}</div>}</div>
          <div style={{ marginBottom:12 }}><label className="label">Téléphone *</label><input className={`input${errors.tel?' err':''}`} type="tel" placeholder={hidePlaceholders?'':'06 XX'} value={form.tel} onChange={e=>setForm(f=>({...f,tel:e.target.value}))} />{errors.tel&&<div className="err">{errors.tel}</div>}</div>
          {isBtob ? (
            <div className="grid2" style={{ marginBottom:12 }}>
              ...
              <div><label className="label">Code postal</label><input className="input" placeholder={hidePlaceholders?'':'06140'} value={form.cp} onChange={e=>setForm(f=>({...f,cp:e.target.value}))} /></div>
            </div>
          ) : (
            <div className="grid2" style={{ marginBottom:12 }}>
              ...
              <div><label className="label">CP</label><input className="input" placeholder={hidePlaceholders?'':'06140'} value={form.cp} onChange={e=>setForm(f=>({...f,cp:e.target.value}))} /></div>
            </div>
          )}
      ...
      {(screen === 'ticket' || screen === 'already') && (
        <div className="screen" style={{ justifyContent:'center',textAlign:'center' }}>
          ...
          <div style={{ fontSize:22,fontWeight:900,marginBottom:6 }}>{screen==='ticket'?(winTitle||'Inscription confirmée !'):'Déjà joué !'}</div>
          ...
          {winCtaPhone && winCtaLabel && (
            <a href={`tel:${winCtaPhone}`} className="btn" style={{ display:'block',textDecoration:'none',width:'82%',margin:'0 auto' }}>{winCtaLabel}</a>
          )}
          {backUrl && (
            <button className="btn-ghost" style={{ display:'block',width:'82%',margin:'10px auto 0' }} onClick={()=>{ try{if(!replayable)localStorage.removeItem(lsKey)}catch{}; if(window.top&&window.top!==window.self){window.top.location.href=backUrl}else{window.location.href=backUrl} }}>← Retour à l&apos;accueil</button>
          )}
        </div>
      )}
    </div>
