...
export default function SpinClient({ event, lots, partenaires, evId }: Props) {
  ...
  const couleur = event?.couleur ?? '#F59E0B'
  const isDark  = false
  ...
  return (
    <div id='flowin-vp' style={vpStyle}>
      ...
      {screen === 'landing' && (
        <div className='fl-screen active' style={{ background: '#fff9f0' }}>
          <div style={{ flex:1, display:'flex', flexDirection:'column', padding:'40px 24px 24px' }}>
            <div style={{ textAlign:'center', marginBottom:32 }}>
              ...
              <div style={{ fontSize:14, color:isDark?'rgba(255,255,255,.6)':'#888', lineHeight:1.5 }}>{event.description || 'Tournez la roue et découvrez votre lot !'}</div>
            </div>
            {lots.length > 0 && (
              <div style={{ marginBottom:24 }}>
                ...
                {lots.slice(0,3).map(lot => (
                  <div key={lot.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'8px 0', borderBottom:`1px solid rgba(0,0,0,.06)` }}>
            ...
            <div style={{ marginTop:'auto' }}>
              <button className='fl-btn fl-btn-primary' style={{ background:couleur, fontSize:17 }} onClick={() => setScreen('form')}>Tourner la roue →</button>
              {partenaires.length > 0 && (
                <button className='fl-btn fl-btn-ghost' style={{ marginTop:10, color:'#F59E0B', borderColor:'#F59E0B' }} onClick={() => setScreen('partenaires')}>🤝 Voir nos partenaires</button>
              )}
            </div>
          </div>
        </div>
