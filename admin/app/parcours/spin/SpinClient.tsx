        ...
        <div className="screen" style={{ justifyContent:'center',alignItems:'center',textAlign:'center',position:'relative',background:'radial-gradient(ellipse at 50% 40%, #0a1f2e 0%, #060d18 70%)' }}>
          ...
          <div style={{ position:'relative',zIndex:10,width:'88%',maxWidth:360,borderRadius:18,overflow:'hidden',border:'2px solid #14B8A6',boxShadow:'0 0 40px rgba(20,184,166,.5),0 12px 48px rgba(0,0,0,.6)',background:'#071620' }}>
            ...
            <div style={{ padding:'22px 20px 26px' }}>
              ...
              <button className="btn-ghost" style={{ display:'block',width:'82%',margin:'10px auto 0',background:'transparent',border:'1px solid rgba(255,255,255,.2)',borderRadius:100,padding:'10px 0',fontWeight:700,color:'rgba(255,255,255,.7)',cursor:'pointer' }} onClick={()=>{ try{localStorage.removeItem(lsKey)}catch{}; if(window.top&&window.top!==window.self){window.top.location.href='https://flowin-events.vercel.app/landing'}else{window.location.href='/landing'} }}>← Retour à l'accueil</button>
              {tirageText && (
