...
export default function QuizClient({ event, lots, partenaires, evId }: Props) {
  ...
  const couleur = event?.couleur ?? '#D4537E'
  const isDark  = false
        ...
        <div className='fl-screen active' style={{ background: '#f8f8fc' }}>
          <div style={{ flex:1, display:'flex', flexDirection:'column', padding:'40px 24px 24px' }}>
            <div style={{ textAlign:'center', marginBottom:32 }}>
              ...
              <div style={{ fontSize:14, color:isDark?'rgba(255,255,255,.6)':'#888', lineHeight:1.5 }}>{event.description || 'Répondez aux questions et tentez de gagner !'}</div>
            </div>
            {lots.length > 0 && (
            ...
            <div style={{ marginTop:'auto' }}>
              <button className='fl-btn fl-btn-primary' style={{ background:couleur, fontSize:17 }} onClick={() => setScreen('form')}>Participer au quiz →</button>
              {partenaires.length > 0 && (
                <button className='fl-btn fl-btn-ghost' style={{ marginTop:10, color:'#D4537E', borderColor:'#D4537E' }} onClick={() => setScreen('partenaires')}>🤝 Voir nos partenaires</button>
              )}
            </div>
          </div>
        </div>
