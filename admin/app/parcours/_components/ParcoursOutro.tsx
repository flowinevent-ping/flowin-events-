...
export default function ParcoursOutro({ superEventId, gain }: { superEventId?: string | null; gain?: GainImmediat | null }) {
  ...
  return (
    <div style={{ position: 'relative', zIndex: 10, width: '88%', maxWidth: 360, margin: '16px auto 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
      {/* Charte NDS 2026 (reference graphique des jeux) : meme carte que la
          carte de gain du gabarit Quiz + bonus. */}
      {gain && (
        <div style={{ background: 'linear-gradient(135deg,#7C2D92,#E0218A)', borderRadius: 16, padding: 16, color: '#fff', boxShadow: '0 8px 24px rgba(124,45,146,.35)', textAlign: 'center' }}>
