...
type Cle = 'date' | 'nom' | 'prenom' | 'adresse' | 'ville' | 'tel' | 'email' | 'event' | 'source' | 'optin' | 'gains'
type Sens = 1 | -1

const fmtDate = (s: string | null) => (s ? new Date(s).toLocaleDateString('fr-FR') : '—')

export default function Page() {
  const { joueurs, events, openDrawer, openDrawerEdit } = useDashboard()
  const [search, setSearch] = useState('')
  const [tri, setTri] = useState<{ cle: Cle; sens: Sens }>({ cle: 'date', sens: -1 })

  const nomEvent = (id: string) => events.find(e => e.id === id)?.nom ?? id
  ...
  const list = useMemo(() => {
    ...
    const val = (j: FlowinJoueur): string | number => {
      switch (cle) {
        ...
        case 'event': return nomEvent(j.events?.[0] ?? '').toLowerCase()
        ...
        <div style={{ overflowX: 'auto' }}>
          <table className="sa-tbl" style={{ width: '100%' }}>
            <thead><tr>
              ...
              <Th cle="event" label="Event" width={150} />
            ...
            <tbody>
              ...
              {list.map((j: FlowinJoueur) => {
                const evs = (j.events ?? []).map(nomEvent)
                return (
                  <tr key={j.id} onClick={() => openDrawer('joueur', j.id)}>
                    ...
                    <td style={{ fontSize: 12 }} title={evs.join(', ')}>
                      {evs.length === 0 ? '—' : evs.length === 1 ? evs[0] : `${evs[0]} +${evs.length - 1}`}
                    </td>
