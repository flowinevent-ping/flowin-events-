  ...
  return (
    <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', fontFamily: 'system-ui, sans-serif', color: '#141a26', background: '#e8eaed' }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 1000, padding: '14px 14px', pointerEvents: 'none' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 13, background: 'linear-gradient(135deg,#2746A6,#3B7DE0)', color: '#fff', boxShadow: '0 6px 22px rgba(39,70,166,.42)', borderRadius: 16, padding: '12px 18px', maxWidth: '90%', pointerEvents: 'auto' }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 13, opacity: 0.88, marginTop: 1 }}>Joue sur place · gagne des lots 🎁</div>
          </div>
        </div>
      </div>

      {/* Onglet bottom-sheet */}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 1000, background: '#fff', borderTopLeftRadius: 22, borderTopRightRadius: 22, boxShadow: '0 -4px 24px rgba(0,0,0,.18)', maxHeight: open ? '70dvh' : undefined, overflowY: open ? 'auto' : 'visible' }}>
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 18, fontWeight: 800 }}>🎮 {lieux.length} commerces à découvrir</span>
        ...
        {open && (
          <div style={{ padding: '4px 20px 28px' }}>
            ...
            {focus && (
              <div className="fe-card" style={{ background: 'linear-gradient(135deg,#0F9E73,#0B6E50)', color: '#fff', borderRadius: 16, padding: '17px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 18, boxShadow: '0 6px 20px rgba(15,158,115,.38)' }}>
                <div>
                  <div style={{ fontSize: 13, opacity: 0.85 }}>Tu es chez</div>
                  <div style={{ fontSize: 21, fontWeight: 800 }}>{focus.nom}</div>
                </div>
                <a href={focus.module ? `/parcours/${focus.module}?ev=${focus.id}` : '#'} style={{ background: '#fff', color: '#0B6E50', fontWeight: 800, fontSize: 16, padding: '13px 22px', borderRadius: 12, textDecoration: 'none', whiteSpace: 'nowrap' }}>
                  Jouer ▸
                </a>
              </div>
            )}
            ...
            {lotsSorted.length > 0 && (
              <div style={{ marginBottom: 22 }}>
                <div style={{ fontSize: 13, fontWeight: 800, letterSpacing: '.06em', color: '#9aa0ad', textTransform: 'uppercase', marginBottom: 12, textAlign: 'center' }}>🏆 À gagner au tirage</div>
                  ...
                  {lotsSorted.slice(0, 3).map((l, i) => {
                    ...
                    return (
                      <div
                        key={l.id}
                        className="fe-card"
                        style={{ flex: 1, background: p.grad, color: p.fg, borderRadius: 16, padding: tall ? '20px 8px' : '15px 8px', textAlign: 'center', ...(i === 0 ? { ['--g' as string]: p.glow, animation: 'feGlow 2.4s ease-in-out infinite, feRise .34s ease both' } : {}) }}
                      >
            ...
            {!focus && (
              <div style={{ display: 'flex', gap: 12, background: '#EFF3FE', borderRadius: 14, padding: '14px 15px', marginBottom: 18, alignItems: 'center' }}>
              ...
              {lieux.map((l, idx) => {
                ...
                return (
                  <div
                    ...
                    className="fe-card"
                    style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 15px', borderRadius: 16, background: '#fff', border: '1px solid #eef0f3', boxShadow: '0 2px 8px rgba(20,26,38,.06)', cursor: 'pointer', animationDelay: `${Math.min(idx * 35, 280)}ms` }}
                  >
                    <span style={{ width: 52, height: 52, borderRadius: 14, background: u.grad, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 27, flexShrink: 0, boxShadow: '0 3px 10px rgba(20,26,38,.14)' }}>{u.emoji}</span>
            ...
            {sponsors.length > 0 && (
              <div style={{ marginTop: 20 }}>
                ...
                <div style={{ display: 'flex', gap: 9, flexWrap: 'wrap' }}>
                  {sponsors.map((s) => (
                    <span key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 6, border: '1px solid #e3e5ea', padding: '6px 12px', borderRadius: 10, fontSize: 13, fontWeight: 500 }}>
      ...
      {selected && fu && (
        <>
          ...
          <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 1600, background: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, boxShadow: '0 -6px 30px rgba(0,0,0,.25)', maxHeight: '86dvh', overflowY: 'auto', animation: 'feSheetUp .3s cubic-bezier(.4,0,.2,1)' }}>

            {/* Hero coloré par jeu */}
            <div style={{ background: fu.grad, color: '#fff', padding: '24px 22px 26px', borderTopLeftRadius: 24, borderTopRightRadius: 24, position: 'relative' }}>
            ...
            <div style={{ padding: '20px 22px 26px' }}>
              <div style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.05em', color: '#9aa0ad', marginBottom: 11 }}>Ce que tu peux gagner ici</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {selected.gain_immediat && (
                  <div className="fe-card" style={{ display: 'flex', alignItems: 'center', gap: 13, background: '#E9F7F0', color: '#0B6E50', borderRadius: 14, padding: '15px 16px' }}>
                    <span style={{ fontSize: 26 }}>🎁</span>
                    <div>
                      <div style={{ fontSize: 12, opacity: 0.8 }}>Gain immédiat</div>
                      <div style={{ fontSize: 17, fontWeight: 800 }}>{selected.gain_immediat}</div>
                    </div>
                  </div>
                )}
                {selected.gain_ticket !== false && (
                  <div className="fe-card" style={{ display: 'flex', alignItems: 'center', gap: 13, background: '#EFF3FE', color: '#2c3a63', borderRadius: 14, padding: '15px 16px', animationDelay: '60ms' }}>
                    <span style={{ fontSize: 26 }}>🎟️</span>
                    <div>
                      <div style={{ fontSize: 12, opacity: 0.8 }}>Tirage au sort final</div>
              ...
              {focus && focus.id === selected.id ? (
                <a href={selected.module ? `/parcours/${selected.module}?ev=${selected.id}` : '#'} style={{ display: 'block', textAlign: 'center', marginTop: 22, background: fu.grad, color: '#fff', fontWeight: 800, fontSize: 18, padding: '17px', borderRadius: 14, textDecoration: 'none', boxShadow: '0 5px 18px rgba(20,26,38,.2)' }}>
                  🎮 Jouer maintenant
                </a>
              ) : (
                <>
                  <div style={{ background: '#FAF6EE', color: '#7a5a1e', borderRadius: 14, padding: '13px 15px', fontSize: 13.5, lineHeight: 1.5, margin: '20px 0 15px' }}>
                  ...
                  {typeof selected.lat === 'number' && typeof selected.lng === 'number' && (
                    <a href={dirUrl(selected.lat, selected.lng)} target="_blank" rel="noopener" style={{ display: 'block', textAlign: 'center', background: HERO, color: '#fff', fontWeight: 800, fontSize: 16.5, padding: '16px', borderRadius: 14, textDecoration: 'none' }}>
                      Y aller · Itinéraire →
                    </a>
                  )}
                </>
