...
export const BLOCS: BlocRoadmap[] = [
  ...
  {
    titre: 'Projet câblage facturation — Jeffrey CRM (18/09, prêt à démarrer sur feu vert de Romain)',
    items: [
      { etat: 'ok', titre: 'Schéma Jeffrey CRM lu et confirmé', detail: 'Projet Supabase pxqeelvavgzfyrwtgmib (accès direct depuis cette session, contrairement à ywcqtupgoxfzkddqkztk). Tables contacts/devis/factures/frais/documents_contact/reglements_facture, toutes tagguées par une colonne `espace`. Table `sequences` (cle/valeur) : compteur par espace pour les contacts (`contact_OPC`, `contact_BAI`...), GLOBAL tous espaces confondus pour devis/factures à ce jour (clés `devis`/`facture` sans suffixe). Vérifié en direct le 18/09 : la base n\\u2019est plus vierge, 3 espaces déjà en production -- Baïta Fenêtres & Vérandas (BAI), Opconsult (OPC), Worknfun (nouvellement identifié, pas encore de code) -- compteur `devis`/`facture` déjà à 3-4, utilisé pour de vraies affaires (ex. dossier "Le Stadium, Riom").' },
      { etat: 'ok', titre: 'Un seul SIRET confirmé pour toutes les marques (18/09)', detail: 'Romain : un seul SIRET, sous l\\u2019égide de BAITA EURL, pour Baïta Fenêtres & Vérandas, Opconsult, Worknfun, ET les sous-marques internes Flowin Event / Flowin Révision / Ping. Aucune de ces entités n\\u2019est une société séparée.' },
      { etat: 'ok', titre: 'Décision d\\u2019architecture de numérotation : option B -- une série continue par marque', detail: 'Romain (18/09) tranche explicitement pour l\\u2019option B : chaque marque/sous-marque a SA PROPRE série de numérotation continue (son propre préfixe + son propre compteur), plutôt qu\\u2019un flux unique mélangé pour tout le monde. Légal en France (numérotation par série) tant que chaque série reste elle-même strictement séquentielle et que la méthode est stable dans le temps -- le SIRET unique BAITA EURL reste porté par les mentions légales du document, pas par le numéro. Avantage direct : FL-2026-0001 déjà émis par Flowin reste valide tel quel, la suite continue proprement (FL-2026-0002...) sans rien renuméroter. Conséquence pour Jeffyre : la table `sequences` doit évoluer d\\u2019un compteur global unique (`devis`/`facture`) vers un compteur par marque (même mécanique que `contact_OPC`/`contact_BAI` existant, à étendre à devis/factures) -- à cadrer techniquement au moment de l\\u2019implémentation.' },
      { etat: 'ok', titre: 'Préfixes de série validés (18/09)', detail: 'Baïta Fenêtres & Vérandas = B ou BT (Romain a cité les deux formes -- à trancher au moment du câblage, BT retenu par défaut pour éviter l\\u2019ambiguïté d\\u2019un préfixe à une seule lettre, sauf contre-ordre de Romain) · Opconsult = OPC · Worknfun = WNF · Flowin Event = FL · Flowin Révision = FLR · Ping = PING.' },
      { etat: 'todo', titre: 'Valeur exacte de `espace` pour Flowin Event', detail: 'Nécessaire avant tout écrit dans Jeffrey (contacts/devis/factures.espace) : Romain doit confirmer la chaîne exacte (ex. "flowin", "flowin-event"...). Toujours en attente au 18/09.' },
      ...
      { etat: 'ok', titre: 'Principe d\\u2019émission : le numéro légal est demandé AVANT génération, jamais poussé après coup', detail: 'Romain a repéré un vrai risque (18/09) : si la sous-marque génère et envoie la facture avec un numéro provisoire puis pousse vers Jeffyre après coup, le document réellement transmis au client ne porte pas le numéro légal. Principe retenu : la sous-marque demande le numéro à Jeffyre en tout premier (appel synchrone, incrémentation atomique du compteur pour éviter toute collision en cas de volume -- critique pour Ping/Stripe), PUIS génère/envoie le document avec ce numéro. Le "numéro interne" (n° de commande, n° de transaction Stripe...) reste une référence technique invisible sur le document, jamais un numéro de facture affiché.' },
      { etat: 'todo', titre: 'Bloqueur confirmé : SSO Vercel sur jeffrey-crm.vercel.app', detail: 'Vérifié via API Vercel (18/09) : le projet jeffrey-crm a ssoProtection.enabled=true, deploymentType="all_except_custom_domains". Les 3 domaines actuels (jeffrey-crm.vercel.app, jeffrey-crm-nextoping.vercel.app, jeffrey-crm-git-main-nextoping.vercel.app) sont tous des sous-domaines vercel.app, donc TOUS protégés par le SSO Vercel de l\\u2019équipe -- un pro externe sans compte dans l\\u2019équipe Vercel de Romain ne peut pas ouvrir un lien direct aujourd\\u2019hui (redirection vers une connexion Vercel). Le câblage direct ne peut pas fonctionner pour un client/pro externe sans l\\u2019un de : (a) un domaine personnalisé sur le projet Jeffrey (non protégé par ce réglage), ou (b) désactiver le SSO pour ce projet, ou (c) une page de partage publique/à jeton côté Jeffyre (lien signé, sans compte, recommandé -- gratuit, ne coûte que du développement, plus sûr que (b) qui exposerait tout le CRM). Réponse de Romain toujours attendue.' },
      { etat: 'todo', titre: 'Recommandation : passer le projet Supabase Jeffyre en plan payant avant mise en prod', detail: 'Vérifié le 18/09 : l\\u2019organisation Supabase "3opconsult" qui héberge Jeffyre est sur le plan gratuit -- aucune garantie de disponibilité, mise en pause automatique après inactivité. Incompatible avec un rôle d\\u2019autorité de numérotation légale appelée en temps réel (en particulier par Ping, à chaque transaction Stripe). À confirmer avec Romain avant bascule en prod.' },
